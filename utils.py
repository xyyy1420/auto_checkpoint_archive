...
def generate_initramfs(specs,elf_suffix,dest_path):
  ...
  for spec in specs:
    spec_files = get_spec_info(def_config()["cpu2006_run_dir"],prepare_config()["elf_folder"],elf_suffix)[spec][0]
...
def generate_run_sh(specs,elf_suffix,dest_path, withTrap=False):
  ...
  for spec in specs:
    spec_bin = get_spec_info(def_config()["cpu2006_run_dir"],prepare_config()["elf_folder"],elf_suffix)[spec][0][0].split("/")[-1]
    spec_cmd = " ".join(get_spec_info(def_config()["cpu2006_run_dir"],prepare_config()["elf_folder"],elf_suffix)[spec][1])

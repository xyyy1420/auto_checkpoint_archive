...
#        res.check_returncode()
    ...
    result={
        "cl_res":os.path.join(cl_res_folder),
        "profiling_log":os.path.join(simpoint_config()["profiling_logs"].format(ptime)),
        "checkpoint_path":os.path.join(os.path.join(def_config()["buffer"],cres_folder)),
        "json_path":os.path.join(def_config()["buffer"],cres_folder,simpoint_config()["json_file"].format(ptime,cltime)),
        "list_path":os.path.join(def_config()["buffer"],cres_folder,simpoint_config()["list_file"].format(ptime,cltime,ctime))
    }
...
def profiling_instrs(profiling_log,spec_app):
    ...
    with open(os.path.join(profiling_log,"{}-out.log".format(spec_app)),"r") as f:
...
def per_checkpoint_generate_json(profiling_log,cluster_path,app_list,target_path):
    ...
    for spec in app_list:
        result.update({spec:{"insts":profiling_instrs(profiling_log,spec),'points':cluster_weight(cluster_path,spec)}})
